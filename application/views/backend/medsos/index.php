...
<?php } else { ?>
<script src="<?=base_url();?>assets/pages/js/form-validation-medsos.js" type="text/javascript"></script>
<script>
var save_method;
var table;
...
$(document).ready(function() {
	...
	table = $('#tableData').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('backend/medsos/loadTable')?>",
            "type": "POST"
        },
...
function save(){ //change button text
	...
	if(save_method == 'add') {
		$('#btnSave').text('Saving...');
        url = "<?php echo site_url('backend/medsos/save')?>";
		$("#msgSKS").text("Data Berhasil Ditambah !!");
		$("#msgERR").text("Data Gagal Ditambah !!");
    } else {
		$('#btnSave').text('Updating...');
        url = "<?php echo site_url('backend/medsos/update')?>";
...
function edit(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('backend/medsos/prepareEdit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			...
			save_method = 'update';
			$("#btnSave").text('Update');
            $('[name="id"]').val(data.medsos_id);
            $('[name="medsos_nama"]').val(data.medsos_nama);
            $('[name="medsos_url"]').val(data.medsos_url);
        },
        error: function (jqXHR, textStatus, errorThrown)
        {
              ...
              <div class="row ">
				<div class="col-md-12">
					<div class="form-group">
						<label>Nama</label>
						<input type="text" class="form-control" name="medsos_nama" readonly>
					</div>
				</div>
				<div class="col-md-12">
					<div class="form-group">
						<label>URL *</label>
						<input type="text" class="form-control" name="medsos_url" maxlength="100">
					</div>
				</div>
			  </div>
			</div>
