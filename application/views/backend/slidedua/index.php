...
$(document).ready(function() {
	...
	table = $('#tableData').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('backend/slidedua/loadTable')?>",
            "type": "POST"
        },
...
function edit(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('backend/slidedua/prepareEdit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			...
			save_method = 'update';
			$("#btnSave").text('Update');
            $('[name="id"]').val(data.data.slide_dua_id);
...
$(document).on("click","#hapusData",function(){
	...
	swal({
		...
		function(){
		 $.ajax({
			url:"<?php echo site_url('backend/slidedua/delete');?>"+'/'+id,
			data: id,
			success: function(data){
				$('#tableData').DataTable().ajax.reload();
...
<section class="content" id="inputan" style="display: none">
    ...
    <section class="content col-md-6">
        <form action="<?=base_url();?>backend/slidedua/uploadFoto" method="post" class="form-data" enctype="multipart/form-data">		
